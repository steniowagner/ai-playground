...
Your response was not a completed investigation.

The final result was rejected because required evidence is missing.
Do not return another final JSON response yet.

Call the appropriate investigation tools now. If the incident record has not
been retrieved, begin by calling get_incident with the incident ID from the
user's request. Continue gathering evidence until the completion criteria are
satisfied.
"""
...
- Use JSON null instead of the string 'None', arrays where arrays are required, and the declared type for every field.
"""
